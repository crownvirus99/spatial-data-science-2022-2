...
disponible <- ee$ImageCollection('LANDSAT/LC08/C01/T1_TOA')$
  filterDate('2016-04-01','2018-07-10')$
  filterBounds(roi)$
...
l8_bands <- ee$Image(escena)$select(c("B4", "B5"))
...
png(file="maule/mongus.png", width=500, height=600)
...
analisis_aculeo <- function(anio){
  ...
  # guardo plot
  png(file=paste0("maule/maule_anio",anio,".png"), width=500, height=600)
